...
│   │   └── ui/                        # 共通コンポーネント（つまみ等）
│   ├── CMakeLists.txt                 # CMake プロジェクト（Projucer は使わない）
│   └── README.md                       # ビルド + 並行作業手順
├── docs/
...
|---|---|---|---|
| **P0** | VS2022 + JUCE インストール、CMake で Plugin プロジェクトをビルド | CMake、`juce_add_plugin`、ターゲット構成、VST3 SDK 配置 | 3〜5日 |
...
- **推奨**: 共通ローカルクローン
  ...
  git checkout 8.0.4
  ```
- 各 worktree でも JUCE_PATH を共有することで重複ダウンロードを防ぐ

### 4.3 JUCE Personal License（無料）
- https://juce.com/get-juce
- 「Personal」を選択 → メール登録
- CMake ビルドだけなら認証は不要のはず（配布する場合は要確認）
- 個人・年商 5万ドル以下: 無料、クローズドソース配布も可

### 4.4 VST3 SDK
- JUCE リポジトリに同梱（`modules/juce_audio_plugin_client/VST3/`）
- 追加ダウンロード不要
...
### 4.6 Visual Studio でデバッグ
- `build\synthesizer_practice.sln` を VS で開く
- `synthesizer_practice_Standalone` をスタートアッププロジェクトに設定 → F5
...
| VS ビルド失敗（最初の山） | CMake 再生成、JUCE タグ固定（8.0.4）、VS の C++ ワークロード確認 |
...
4. **P0 開始**: CMake でビルド → スタンドアロン起動確認
...
| 2026-06-15 | プロジェクト管理: Projucer → **CMake** に変更（並行開発で衝突回避） |
| 2026-06-15 | リポジトリ構造変更を実施（既存を `web/` に移動） |
| 2026-06-15 | desktop プロジェクト基盤（CMakeLists.txt + Source/ スタブ）作成 |
| 2026-06-15 | 詳細設計書 `docs/desktop-design.md` 作成、TASKS/README 整備 |
